...
class ChatDataCleaner:
    ...
    def should_drop_content(self, content):
        ...
        # Drop placeholder square-bracket tokens like [Happy], [Surprise]
        if self._bracket_token_re.search(content):
            return True

        # Drop mentions and obvious placeholders
        if "@" in content:
            return True
    ...
    def clean_and_format_chat_data(self):
        ...
        # 2. 核心重构：严格交替的多轮对话构建
        finetune_data = []
        SYSTEM_PROMPT = f"你是{TARGET_ASSISTANT}，一个幽默且带有独特风格的聊天伙伴。"

        for session in conversations:
            if not session:
                continue

            # 步骤 A：合并连续的同角色发言（解决“碎碎念”问题）
            merged_messages = []
            for msg in session:
        ...
        print(f"- 生成高质量多轮对话数: {len(finetune_data)} 个")
        print(f"- 结果已保存至: {self.output_file}")


if __name__ == "__main__":
    raw_data_file = "chat_history.jsonl"
    output_filename = "MiliBot_sharegpt.json"
